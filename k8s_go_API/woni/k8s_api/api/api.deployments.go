...
// #1. deployment 조회
func ListDeployment(clientset *kubernetes.Clientset, namespace string) []appsv1.Deployment {
...
// #2. deployment 생성
func CreateDeployment(clientset *kubernetes.Clientset, dp DeploymentStruct) {
	fmt.Println("createDeployment fun - dp : ", dp.DNAME, dp.DNS)
	// deployment 구성하기 - 매니페스트 설정

	deploymentsClient := clientset.AppsV1().Deployments(dp.DNS)
	///삭제를요함!!!!!!!!!!!!!!!!!!!!1-------------------------------------------
	//새로 생성하는 것이므로 매니페스트 설정이 필요하다 !! deploymentsClient typev1.DeploymentInterface, >>  면한꺼번에 묶으려
	dpManifest := InfoDeployment(dp)

	//생성 Deployment
	// true : 바로 deployment 생성 false : namespace 별도 구현
	if !LsNamespace(clientset, dp.DNS) {
		CreateNamespace(clientset, dp.DNS)
	}
	result, err := deploymentsClient.Create(context.TODO(), dpManifest, metav1.CreateOptions{})
	Checkerror(err)
	fmt.Printf("Created deployment %q.\n", result.GetObjectMeta().GetName())
	//eturn result.GetObjectMeta().GetName()
...
// #4. deployment 삭제
func DeleteDeployment(clientset *kubernetes.Clientset, dp DeploymentStruct) {
	deploymentsClient := clientset.AppsV1().Deployments(dp.DNS)

	deletePolicy := metav1.DeletePropagationForeground
	if err := deploymentsClient.Delete(context.TODO(), dp.DNAME, metav1.DeleteOptions{
		PropagationPolicy: &deletePolicy,
	}); err != nil {
		panic(err)
	}
	fmt.Println("Deleted deployment : ", dp.DNAME)
}

//매니페스트 정보생성
func InfoDeployment(dp DeploymentStruct) (deployment *appsv1.Deployment) {
	...
	fmt.Println("infodeployment_struct")
	return
}
